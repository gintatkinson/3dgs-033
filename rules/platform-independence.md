...
- **Mermaid Relationship Label Rules**: Relationship labels containing spaces MUST be enclosed in double quotes (e.g., use `RootContainer *-- NestedContainer : "contains a nested container"`, not `RootContainer *-- NestedContainer : contains a nested container`). Colons are strictly prohibited in relationship labels — quoted or not — because the GitHub Mermaid renderer rejects them regardless of quoting (upstream issue #333). Replace module-prefix colons with dashes or remove them entirely (e.g., use `"augments network node"` instead of `"augments nw:node"`). A single unquoted word is permitted (e.g., `A --> B : references`).
